{
  ...
  flake.wrappersModules.kitty = {
    ...
    config = {
      settings = lib.optionalAttrs (config.shell != "") { shell = config.shell; } // {
  ...
  perSystem = {pkgs, ...}: let
    wrappedKitty = inputs.wrapper-modules.wrappers.kitty.wrap {
      inherit pkgs;
      imports = [self.wrappersModules.kitty];
    };
    origKitty = pkgs.kitty;
  in {
    packages.kitty = pkgs.symlinkJoin {
      name = "kitty-wrapped-${origKitty.version}";
      paths = [wrappedKitty origKitty];
      meta.mainProgram = "kitty";
      postBuild = ''
        for f in $out/share/applications/*.desktop; do
          sed -i "s|^Exec=kitty|Exec=$out/bin/kitty|g; s|^TryExec=kitty|TryExec=$out/bin/kitty|g" "$f"
        done
      '';
    };
  };
